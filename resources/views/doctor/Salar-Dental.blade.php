@extends('layouts.content')
@section('content')
  <div class="page-heading">
    <div class="container">
      <div class="row">
        <div class="col-lg-8 offset-lg-2 header-text">
          <h2><em>Salar </em> Dental</h2>
          <p>  بكالوريوس جراحة الأسنان (BDS) - ماستر في معالجة وتجميل الأسنان </p>
        </div>
      </div>
    </div>
  </div>

  <div class="top-categories">
    <div class="container">
      <div class="row">
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-15.png" alt="">
            </div>
            <h4>فحص مجاني</h4>
            <span>الخدمات المجانية</span>
            <span class="counter">نسبة الخصم 100</span>
          </div>
        </div>
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-15.png" alt="">
            </div>
            <h4> x ray  صورة</h4>
            <span>الخدمات المجانية</span>
            <span class="counter">نسبة الخصم 100</span>
          </div>
        </div>
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-15.png" alt="">
            </div>
            <h4>pa صورة </h4>
            <span>الخدمات المجانية</span>
            <span class="counter">نسبة الخصم 100</span>
          </div>
        </div>
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-15.png" alt="">
            </div>
            <h4> غسل أسنان </h4>
            <span>الخدمات المجانية</span>
            <span class="counter">لشخص واحد للعائلة</span>
          </div>
        </div>
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-15.png" alt="">
            </div>
            <h4> بولش أسنان </h4>
            <span>الخدمات المجانية</span>
            <span class="counter">لشخص واحد للعائلة</span>
          </div>
        </div>
      </div>
    </div>
  </div>
  <section class="featured-contests">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="section-heading text-center">
            <h6>قائمة الأسعار</h6>
            <h4>احصل على تخفيضات <em>و خدمات مجانية</em> احجز <em>الان</em></h4>
          </div>
          <div>
          <div class="rtl-container">
            <table class="table table-info text-center">
              <thead>
                <tr>
                  <th scope="col">السعر بعد التخفيض</th>
                  <th scope="col">السعر</th>
                  <th scope="col">الخدمات</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>35.000</td>
                  <td>60.000</td>
                  <td>حشوه</td>
                </tr>
                <tr>
                  <td>80.000</td>
                  <td>120.000</td>
                  <td>حشوة عصب اماميه</td>
                </tr>
                <tr>
                  <td>100.000</td>
                  <td>140.000</td>
                  <td>حشوة عصب اماميه</td>
                </tr>
                <tr>
                  <td>100.000</td>
                  <td>150.000</td>
                  <td>كراس- تلبيس سيراميك</td>
                </tr>
                <tr>
                  <td>150$</td>
                  <td>200$</td>
                  <td>كراس- تلبيس زيركون</td>
                </tr>
                <tr>
                  <td>15.000</td>
                  <td>25.000</td>
                  <td>قلع سن</td>
                </tr>
                <tr>
                  <td>100.000</td>
                  <td>150.000</td>
                  <td>قلع سن جراحي</td>
                </tr>
                <tr>
                  <td>40.000</td>
                  <td>60.000</td>
                  <td>حشوة اطفال</td>
                </tr>
                <tr>
                  <td>100$</td>
                  <td>200$</td>
                  <td>تبييض ليزر</td>
                </tr>
                <tr>
                  <td>750.000</td>
                  <td>1000.000</td>
                  <td>طقم كامل</td>
                </tr>
                <tr>
                  <td>1000$</td>
                  <td>1600$</td>
                  <td>تقويم</td>
                </tr>
                <tr>
                  <td>500$</td>
                  <td>600$</td>
                  <td>زرع- جاندني ددان سويسري</td>
                </tr>
              </tbody>
            </table>
          </div>
          </div>
        </div>
      </div>
  </section>      
  
  <section class="featured-contests">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="section-heading text-center">
            <h6>معرض الصور</h6>
            <h4>سەنتەرێ جوانکاری و چاندن و راستکرنا ددانا </h4>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img70.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img71.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img72.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img73.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img74.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img76.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img77.jpg" alt="">
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="testimonials">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="section-heading text-center">
            <h6>الأطباء المتخصصين</h6>
            <h4>أفضل <em>الأطباء</em>ذات الخبرات العلمية </h4>
          </div>
        </div>
        <div class="col-lg-8 offset-lg-2">
          <div class="owl-testimonials owl-carousel">
            <div class="item">
              <div class="content">
                <div class="left-content">
                  <p>
                  بكالوريوس جراحة الأسنان (BDS)
                  ماستر في معالجة وتجميل الأسنان
                  </p>
                  <h4>د . سالار</h4>
                  <span>07507680625</span>
                </div>
                <div class="image">
                  <img src="assets/images/author15.jpg" style="width:225px">
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

@endsection