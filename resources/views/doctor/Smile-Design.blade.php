@extends('layouts.content')
@section('content')
  <div class="page-heading">
    <div class="container">
      <div class="row">
        <div class="col-lg-8 offset-lg-2 header-text">
          <h2><em>Smile Design </em> Dental</h2>
          <p>  بكالوريوس جراحة الأسنان (BDS) - ماستر في معالجة وتجميل الأسنان </p>
        </div>
      </div>
    </div>
  </div>

  <div class="top-categories">
    <div class="container">
      <div class="row">
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-04.png" alt="">
            </div>
            <h4>فحص مجاني</h4>
            <span>الخدمات المجانية</span>
            <span class="counter">نسبة الخصم 100</span>
          </div>
        </div>
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-04.png" alt="">
            </div>
            <h4>pa- X-ray صورة </h4>
            <span>الخدمات المجانية</span>
            <span class="counter">نسبة الخصم 100</span>
          </div>
        </div>
        <div class="col-lg col-sm-4">
          <div class="item">
            <div class="icon">
              <img src="assets/images/icon-04.png" alt="">
            </div>
            <h4> غسل و بولش أسنان </h4>
            <span>الخدمات المجانية</span>
            <span class="counter">لشخص واحد للعائلة</span>
          </div>
        </div>
      </div>
    </div>
  </div>
  <section class="featured-contests">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="section-heading text-center">
            <h6>قائمة الأسعار</h6>
            <h4>احصل على تخفيضات <em>و خدمات مجانية</em> احجز <em>الان</em></h4>
          </div>
          <div>
          <div class="rtl-container">
            <table class="table table-info text-center">
              <thead>
                <tr>
                  <th scope="col">السعر بعد التخفيض</th>
                  <th scope="col">السعر</th>
                  <th scope="col">الخدمات</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>140.000</td>
                  <td>180.000</td>
                  <td>حشوة عصب</td>
                </tr>
                <tr>
                  <td>50.000</td>
                  <td>75.000</td>
                  <td>حشوة عادي(تجميلي)</td>
                </tr>
                <tr>
                  <td>80$</td>
                  <td>100$</td>
                  <td>تغليف سيراميك</td>
                </tr>
                <tr>
                  <td>100$</td>
                  <td>150$</td>
                  <td>تغليف زيركون</td>
                </tr>
                <tr>
                  <td>170$</td>
                  <td>200$</td>
                  <td>تغليف ايماكس</td>
                </tr>
                <tr>
                  <td>200$</td>
                  <td>250$</td>
                  <td>تغليف زركون(مختبر دبي)</td>
                </tr>
                <tr>
                  <td>270$</td>
                  <td>300$</td>
                  <td>تغليف ايماكس(مختبر دبي)</td>
                </tr>
                <tr>
                  <td>1.200.000</td>
                  <td>1.500.000</td>
                  <td>طخم أسنان</td>
                </tr>
                <tr>
                  <td>20.000</td>
                  <td>25.000</td>
                  <td>الاشعه بانوراما</td>
                </tr>
                <tr>
                  <td>650$</td>
                  <td>800$</td>
                  <td>زرع- جاندني ددان أسنان</td>
                </tr>
                <tr>
                  <td>1200$</td>
                  <td>1400$</td>
                  <td>تقويم الأسنان</td>
                </tr>
                <tr>
                  <td>20.000</td>
                  <td>25.000</td>
                  <td>قلع الأسنان</td>
                </tr>
              </tbody>
            </table>
          </div>

          </div>
          </div>
        </div>
      </div>
  </section>      
  
  <section class="featured-contests">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="section-heading text-center">
            <h6>معرض الصور</h6>
            <h4>سەنتەرێ جوانکاری و چاندن و راستکرنا ددانا </h4>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img61.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img66.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img62.jpg" alt="">
            </div>
          </div>
        </div>

        <div class="col-lg-6">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img64.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-8">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img65.jpg" alt="">
            </div>
          </div>
        </div>
        <div class="col-lg-4">
          <div class="item">
            <div class="thumb">
              <img src="assets/images/img63.jpg" alt="">
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="testimonials">
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <div class="section-heading text-center">
            <h6>الأطباء المتخصصين</h6>
            <h4>أفضل <em>الأطباء</em>ذات الخبرات العلمية </h4>
          </div>
        </div>
        <div class="col-lg-8 offset-lg-2">
          <div class="owl-testimonials owl-carousel">
            <div class="item">
              <div class="content">
                <div class="left-content">
                  <p>
                  بكالوريوس جراحة الأسنان (BDS)
                  ماستر في معالجة وتجميل الأسنان
                  </p>
                  <h4>د.تابان شامل أمين</h4>
                  <span>07504067770</span>
                </div>
                <div class="image">
                  <img src="assets/images/img60.jpg" style="width:225px">
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

@endsection